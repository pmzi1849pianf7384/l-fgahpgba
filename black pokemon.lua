	...
	if Player and UseDrawingLib and not Colour then
		...
		if Player then
			if not Blacklisted then 
				...
				TextLabel.Text = "[Loser faggot] "..Player.Name.." (Loser fag)"
			end 
		else 
			TextLabel.Text = Name
		end
...
local Mouse1 = LP.Character.GetMouse
local OldIndex = nil

OldIndex = hookmetamethod(game, "__index", function(Self, Key)
    if Aimlock and AimlockTarget ~= nil and not checkcaller() and Key == "Hit" and Self == Mouse or Self == Mouse1 then
        return AimbotToCFrame()
    end
...
end,"sleepyairwalk",{"saw"},"airwalk that fixes fly glitching","[sleepyxcory]")
AddCommand(function(Arguments)
...
AddCommand(function(Arguments)
	...
	Input.InputBegan:Connect(
		function(hotkey)
			if hotkey.KeyCode == Enum.KeyCode.L then
				if Input:GetFocusedTextBox() == nil then
					 local SavePos = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
					ammo = true
				end
	...
	game:GetService('RunService').Stepped:connect(
		function()
			if ammo then
					 if game.Workspace:FindFirstChild("Burger | $15") then
					game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
						game.Workspace["Burger | $15"].Head.CFrame
				end
			   end
			game.Players.Localplayer.Backpack.ServerTraits.Stann.Value = 100
		end
	...
	Input.InputBegan:Connect(
		function(hotkey)
			if hotkey.KeyCode == Enum.KeyCode.H then
				if Input:GetFocusedTextBox() == nil then
					 local SavePos = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
					ammo = true
				end
	...
	end,"ammokeys",{"aks"},"do this command for ammo keys","[Player]")
...
AddCommand(function(Arguments)
	...
	getgenv().BlinkKey = "E" -- Case sensitive due to UserInputService, for example if its "e" make sure its "E";
	...
	local Blink, BlinkSpeed, KeyHeld, tableConcatenate = false, 7, false, table.concat;
	...
	Commands["Prefix"] = {
		["Aliases"] = {"prefix"};
		["Description"] = "Sets your Chat refix [.prefix -]";
		["Function"] = function(args)
			if args[1] then 
				Prefix = args[1]
			end
	...
	end,"smooth",{"sb"},"turns on smooth blinkspeed, change speed with the chat prefix .bs","[Number]")
...
AddCommand(function()
	...
	local IsA = game.IsA
local Chairs = workspace:GetDescendants()
for I = 1, #Chairs do
    local Seats = Chairs[I]
    if IsA(Seats, 'Seat') then
        Seats:Destroy()
    end
end
end,"neversit",{"ns"},"Toggles the possibility of you being able to sit down","[No Args]")
...
AddCommand(function(Arguments)
    if Arguments[1] then 
        ...
        notif("CamlockTarget","Has been set to "..CamlockTarget,2,nil)
    end
end,"camlocktarget",{"ctarget"},"Head,Torso,HumanoidRootPart to switch the camlock target","[Head/Torso/HumanoidRootPart]")
...
end,"camlock",{"c"},"Different type of aimbot (Uses camera instead of the remote)","[Player Name]")
        ...
        if game.Players.LocalPlayer.Character.Humanoid:findFirstChild("Bullet") then
            ...
            if game.Players.LocalPlayer.Character.Humanoid.Bullet:findFirstChild("Trail") then
                ...
                if game.Players.LocalPlayer.Character.Humanoid:findFirstChild("Bullet"):findFirstChild("Trail").Lifetime < 0.21 then
                    ...
                    game.Players.LocalPlayer.Character.Humanoid:findFirstChild("Bullet").Trail.Color = ColorSequence.new(Color3.fromRGB(211,88,33),Color3.fromRGB(0, 0, 1))Color3.fromRGB(211,88,33)Color3.fromRGB(248, 255, 0)Color3.fromRGB(211,88,33)Color3.fromRGB(248, 255, 0)
...
end,"aimtarget",{"aimt"},"Changes the aim target [Head/Torso/Prediction]","[Head/Torso/Prediction]")
...
AddCommand(function(Arguments)
	if Arguments[1] and Arguments[1] ~= "all" then
		...
		if Player and Player ~= LP and tostring(AimlockTarget) ~= tostring(Player) then 
			...
			notif("AimlockTarget","Locked onto "..AimlockTarget.Name,5,nil)
		end
	else
		Aimlock = not Aimlock
...
coroutine.resume(coroutine.create(function()
	while wait(1) do
		if CmdBarKey == "Quote" then 
			HotkeysTextLabel.Text = " Vamp"
		else
			HotkeysTextLabel.Text = " Vamp "..CmdBarKey.." Vamp"
		end
...
notif("Cory's cyrus remake","Loaded in " .. string.format("%.6f",tick()-Tick) .. " seconds",10,"Loser") 
